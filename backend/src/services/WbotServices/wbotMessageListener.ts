...
export const flowbuilderIntegration = async (
  ...
  if (ticket.flowWebhook && ticket.lastFlowId && msg) {
    console.log(`[CAMPANHAS] Ticket ${ticket.id} já em fluxo ativo (lastFlowId: ${ticket.lastFlowId}), ignorando nova verificação de campanha`);
    return false;
  }
...
if (!msg.key.fromMe && !ticket.imported && !isGroup && ticket.isBot !== false) {
    ...
    try {
      ...
      if (campaignExecuted) {
        console.log("[HANDLE MESSAGE] ✅ Campanha executada, parando outros fluxos");
...
if (
    ...
    setTimeout(async () => {
      try {
        ...
        logger.error("[CAMPAIGN MESSAGE] Erro ao verificar campanhas:", error);
      }
    }, 1000);
  }
